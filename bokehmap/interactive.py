""" 
Client side test for bokeh animations, with sliders.

@author: Nick Gibbons
"""

# You must first run "bokeh serve" to view this example

from numpy import pi, cos, sin, linspace, roll
from os.path import dirname, join
from bokeh.client import push_session
from bokeh.io import curdoc
from bokeh.plotting import figure
from bokeh.layouts import layout, widgetbox, row
from bokeh.models.widgets import Slider, Select, TextInput
from bokeh.models import Div

M = 5
N = M*10 + 1
r_base = 8
theta = linspace(0, 2*pi, N)
r_x = linspace(0, 6*pi, N-1)
rmin = r_base - cos(r_x) - 1
rmax = r_base + sin(r_x) + 1

desc = Div(text=open(join(dirname(__file__), "description.html")).read(), width=800)

colors = ["FFFFCC", "#C7E9B4", "#7FCDBB", "#41B6C4", "#2C7FB8", "#253494", "#2C7FB8", "#41B6C4", "#7FCDBB", "#C7E9B4"] * M

slider = Slider(title="Time", value=80, start=10, end=300, step=10)

# figure() function auto-adds the figure to curdoc()
p = figure(x_range=(-11, 11), y_range=(-11, 11))
r = p.annular_wedge(0, 0, rmin, rmax, theta[:-1], theta[1:],
                    fill_color=colors, line_color="white")

# open a session to keep our local document in sync with server

ds = r.data_source

def time_select():
    val = slider.value
    rmin = roll(ds.data["inner_radius"], val)
    rmax = roll(ds.data["outer_radius"], -val)
    ds.data.update(inner_radius=rmin, outer_radius=rmax)
    return 


def update():
    time_select()
    return

controls = [slider]

for control in controls:
    control.on_change('value', lambda attr, old, new: update())

inputs = widgetbox(controls)
curdoc().add_root(row(inputs, p, width=800))
curdoc().title = "Sliders"
#curdoc().add_periodic_callback(update, 30)
#curdoc().add_root(1)


